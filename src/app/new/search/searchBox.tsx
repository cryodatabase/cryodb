...
export function SearchBox() {
  ...
  const handleSearchClick = () => {
    ...
    window.location.href = `/new/search/${encodedSearchTerm}`
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "Enter" && focusedIndex === -1) {
      ...
      event.preventDefault()
      window.location.href = `/new/database/${encodeURIComponent(results[focusedIndex].preferred_name)}`
      setIsDropdownOpen(false)
          ...
          >
            {(isTyping || isFetching) ? (
              ...
              results.map((result, index) => (
                <Link
                  href={`/new/database/${encodeURIComponent(result.preferred_name)}`}
                  key={index}
                  className={cn(
